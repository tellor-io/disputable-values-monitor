...
async def dispute(cfg: TelliotConfig, account: ChainedAccount, new_report: NewReport) -> None:
    ...
    tx_receipt, status = await token.write("approve", spender=governance.address, amount=dispute_fee, gas_limit=35000, max_priority_fee_per_gas=gas_price)

    if not status.ok:
        logger.error("unable to approve tokens for dispute fee:" + status.error)
        return None

    logger.info("Approval Tx Receipt: ", tx_receipt)
    ...
    tx_receipt, status = await governance.write(func_name="beginDispute", _queryId=new_report.query_id, _timestamp=new_report.submission_timestamp, gas_limit=60000, max_priority_fee_per_gas=gas_price)

    if not status.ok:
        logger.error(f"unable to begin dispute on {new_report.query_id} at submission timestamp {new_report.submission_timestamp}:" + status.msg)
        return None

    logger.info("Dispute Tx Receipt: ", tx_receipt)
