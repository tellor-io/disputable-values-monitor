...
from tellor_disputables.data import get_contract, parse_new_report_event
...
async def test_disputability_mimicry_nft_index_type(setup, disputer_account):
    ...
    first_submit_txn = tellorflex.functions.submitValue(_queryId=mimicry_nft_market_index_usd_feed.query.query_id, _value=value, _nonce=0, _queryData=mimicry_nft_market_index_usd_feed.query.query_data).buildTransaction({
        "gas": 350000,
        "gasPrice": w3.eth.gas_price,
        "nonce": w3.eth.get_transaction_count(wallet),
        "from": wallet
    ...
    second_submit_txn = tellorflex.functions.submitValue(_queryId=mimicry_nft_market_index_usd_feed.query.query_id, _value=bad_val_encoded, _nonce=0, _queryData=mimicry_nft_market_index_usd_feed.query.query_data).buildTransaction({
        "gas": 350000,
        "gasPrice": w3.eth.gas_price,
        "nonce": w3.eth.get_transaction_count(wallet),
        "from": wallet
    ...
    with patch("telliot_feeds.sources.mimicry.nft_market_index.NFTGoSource.fetch_new_datapoint", AsyncMock(side_effect=lambda: (val, datetime_now_utc()))):
        parse_first_event = await parse_new_report_event(cfg, monitored_feeds=AutoDisputerConfig().monitored_feeds, log=first_receipt["logs"][1])
        parse_second_event = await parse_new_report_event(cfg, monitored_feeds=AutoDisputerConfig().monitored_feeds, log=second_receipt["logs"][1])
        # good report
